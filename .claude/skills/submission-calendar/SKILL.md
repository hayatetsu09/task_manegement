...
**Outlook / 大学メール** — Microsoft 365 コネクタのメール検索ツールが使えるなら、
同じ趣旨のキーワード（提出 / 課題 / レポート / 締切 / 期限 / 必着 など）で検索して本文を取る。
コネクタが無い場合は Gmail だけで続け、**報告にその旨を 1 行書く**（黙って省略しない）。
...
- `source` は Gmail なら `"gmail"`、Outlook なら `"outlook"`。
